...
TBC ist ein modularer, Docker-basierter Kamera-Manager. Hersteller werden über installierbare Kamera-Module angebunden. Reolink, Aqara, TP-Link/Tapo, Axis, Foscam, Hikvision, Dahua (inkl. Amcrest/Annke-OEMs), Ubiquiti/UniFi Protect und SONOFF stehen als direkt installierbare Standard-Repositories bereit; eingebaut bleiben ein reines RTSP-Profil und ein herstellerneutraler Standard-ONVIF-Fallback. Die aktuelle Version bringt Login, Kamera-Verwaltung, RTSP-Stream-Ermittlung, Dashboard-Vorschaubilder, ereignisbasierte Aufnahmen, Clip-Browser, Rollen, MQTT/Home-Assistant-Anbindung, Live-HLS, Retention, Benachrichtigungen, Health-Monitoring und NVR-Kanalverwaltung mit.
...
Unter `Admin → Externe Quellen` können die vorkonfigurierten Hersteller-Repositories mit einem Klick registriert und installiert werden. Die vollständige Liste steht in [docs/plugin-sources.md](docs/plugin-sources.md).
...
Neben direkt verbundenen Kameras kann TBC auch Hersteller-Cloud-Konten anbinden, die mehrere Geräte gleichzeitig verwalten (z. B. UniFi Protect). Cloud-Konten sind als eigenes Plugin-System umgesetzt, getrennt von den Kamera-Modulen: Ein Cloud-Plugin meldet sich einmal am Konto an und listet dabei alle zugehörigen Kameras auf; gefundene Geräte werden anschließend als normale Kameras angelegt und nutzen ab dann dieselbe Live-/Steuerungs-Infrastruktur wie jede andere Kamera. Cloud-Konten werden - wie oben beschrieben - über „Kamera hinzufügen → Cloud-Konto“ erreicht, nicht über den Admin-Bereich, da es sich inhaltlich um eine Art der Kamera-Einrichtung handelt.

Im Adminbereich `Cloud-Anbieter` werden Cloud-Plugins importiert, exportiert und entfernt (identisch zu `Kamera-Plugins`); auf der Seite `Cloud-Konten` selbst werden tatsächliche Konten angelegt, die Verbindung getestet und Geräte gesucht. Verlangt ein Anbieter beim Login einen Bestätigungscode (2FA/E-Mail/SMS), führt TBC statt einer Fehlermeldung auf eine eigene Bestätigungsseite mit genau einem Eingabefeld; jedes Cloud-Plugin meldet im Adminbereich `Cloud-Anbieter` verbindlich, ob es das jemals braucht. Ausgeliefert sind die Plugins `unifi_protect` (Ubiquiti UniFi Protect, lokal oder über die `ui.com`-Cloud-Konsole), `eufy` (Eufy Security, inklusive E-Mail-Bestätigungscode-Flow) und `ewelink` (eWeLink/SONOFF über die offizielle CoolKit-Open-Platform-API; benötigt eine kostenlose App-ID/App-Secret-Registrierung, liefert aber keine automatischen RTSP-Links). Importierte Cloud-Plugins liegen dauerhaft unter `TBC_CLOUD_MODULES_PATH` (standardmäßig `/data/cloud-modules`). Die technische Anleitung zur Entwicklung zusätzlicher Cloud-Plugins steht in [docs/cloud-accounts.md](docs/cloud-accounts.md).

## Externe Quellen, Updates und Plugin-Tests

Kamera-Plugins, Cloud-Plugins und Designs müssen nicht mehr per manuellem ZIP-Upload installiert werden: Unter `Admin → Externe Quellen` registriert man ein öffentliches GitHub-Repository (optional mit Branch/Tag und Unterordner für Mono-Repos), und ein Klick auf „Synchronisieren“ installiert das Plugin direkt daraus - über exakt denselben Sicherheitspfad wie ein manueller ZIP-Upload (Manifest-Prüfung, Pfad-Traversal-Schutz, Dateityp- und Größenlimits). Dieselbe Seite beschreibt die erforderliche Plugin-Struktur direkt in der Oberfläche und bietet für jede Plugin-Art eine fertige, installierbare Vorlage zum Herunterladen an. Jede registrierte Quelle wird zusätzlich stündlich automatisch auf ein neueres Update geprüft (nur eine Commit-SHA-Abfrage, kein Download); steht ein Update bereit, erscheint es unter `Admin → Updates` mit einer Zähler-Anzeige im Menü (z. B. „Updates (2)“) und kann dort einzeln installiert werden. Kamera- und Cloud-Plugins dürfen außerdem einen eigenen `tests/`-Ordner mitbringen; ein „Tests ausführen“-Knopf auf den jeweiligen Plugin-Verwaltungsseiten startet ihn direkt in der Weboberfläche. Details stehen in [docs/plugin-sources.md](docs/plugin-sources.md).
